...
import type {
  CreateQuestionData,
  Question,
} from "@/hooks/free-exam-hooks/use-questions";
import { useState } from "react";
import { AutoQuestionParser } from "./AutoQuestionParser";
...
export const QuestionForm: React.FC<QuestionFormProps> = ({
  ...
  const initialCorrect =
    initialData?.options?.find((opt) => opt.isCorrect)?.optionOrder ||
  ...
  const handleParsedQuestion = (parsed: ParsedQuestion) => {
    // Update form fields with parsed data
    form.setValue("question", parsed.question);
    // Replace options with parsed options, mapping to the expected structure
    replace(
      parsed.options.map((optionText) => ({
        optionText,
        description: "",
        image: "",
      }))
    );
    // Set default correct option to 1 since parser doesn't provide this
    form.setValue("correctOption", 1);
    setParseError("");
  };
  ...
  const difficultyOptions = [
    ...
    (opt) => opt.value === watched.difficulty
  );
                ...
                <Card className="bg-zinc-800/50 border-zinc-700">
                  <CardContent className="p-4 space-y-4">
                    <div>
                      ...
                      {watched.description && (
                        <p className="text-sm text-zinc-400 mb-2">{watched.description}</p>
                      )}
                    </div>
